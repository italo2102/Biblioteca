...
class Checkout(models.Model):
    ...
    member_id = fields.Many2one("library.member", string="Usuarios", required=True)
    user_id = fields.Many2one("res.users", "Bibliotecario", default=lambda s: s.env.user)
    line_ids = fields.One2many(
        "library.checkout.line",
        "checkout_id",
        string="Libros Prestados",
    )
    #author_ids = fields.Many2many('library.author', string='Autores', related='book_id.author_ids')
    ...
    def _compute_num_books(self):
        for libro in self:
            libro.num_books = len(libro.line_ids)
    ...
    def create(self, vals):
        ...
        if new_record.stage_id.state in ("open", "done"):
            raise exceptions.UserError(
                "Estado no disponible para nuevos pedidos."
            )
        return new_record
